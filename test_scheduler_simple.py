#!/usr/bin/env python3
"""
Simplified Scheduler Performance Testing Framework
Tests all schedulers with 5k jobs and analyzes their performance (text-only)
"""
...
class SchedulerTester:
    ...
    def generate_report(self) -> str:
        ...
        if best_throughput.scheduler_name == best_wait_time.scheduler_name:
            ...
            report.append(f"⏱️  For minimum wait times: {best_wait_time.scheduler_name}")
        
        report.append(f"🖥️  For maximum GPU utilization: {best_gpu_util.scheduler_name}")
        report.append(f"⚖️  For fairest scheduling: {best_fairness.scheduler_name}")
        
        return "\n".join(report)
    
    def create_text_charts(self):
        """Create simple text-based performance charts"""
        if not self.results:
            print("No results to visualize")
            return
        
        print("\n" + "=" * 80)
        print("PERFORMANCE CHARTS (Text-based)")
        print("=" * 80)
        
        # Throughput comparison
        print("\n📊 THROUGHPUT COMPARISON (jobs/hour)")
        print("-" * 50)
        max_throughput = max(r.throughput for r in self.results.values())
        for name, result in self.results.items():
            bar_length = int((result.throughput / max_throughput) * 40)
            bar = "█" * bar_length + "░" * (40 - bar_length)
            print(f"{name:<20} {bar} {result.throughput:>8.1f}")
        
        # Wait time comparison (lower is better)
        print("\n⏱️  WAIT TIME COMPARISON (seconds)")
        print("-" * 50)
        max_wait = max(r.avg_wait_time for r in self.results.values())
        for name, result in self.results.items():
            bar_length = int((1 - result.avg_wait_time / max_wait) * 40)
            bar = "█" * bar_length + "░" * (40 - bar_length)
            print(f"{name:<20} {bar} {result.avg_wait_time:>8.1f}")
        
        # GPU utilization comparison
        print("\n🖥️  GPU UTILIZATION COMPARISON (%)")
        print("-" * 50)
        for name, result in self.results.items():
            bar_length = int((result.gpu_utilization / 100) * 40)
            bar = "█" * bar_length + "░" * (40 - bar_length)
            print(f"{name:<20} {bar} {result.gpu_utilization:>8.1f}")
        
        # Fairness comparison (lower is better)
        print("\n⚖️  FAIRNESS COMPARISON (variance - lower is better)")
        print("-" * 50)
        max_fairness = max(r.fairness_score for r in self.results.values())
        for name, result in self.results.items():
            bar_length = int((1 - result.fairness_score / max_fairness) * 40)
            bar = "█" * bar_length + "░" * (40 - bar_length)
            print(f"{name:<20} {bar} {result.fairness_score:>8.1f}")
...
def main():
    ...
    # Create tester
    tester = SchedulerTester(num_jobs=100)  # Start with 100 for testing
    ...
    try:
        ...
        with open('scheduler_detailed_results.csv', 'w', newline='') as csvfile:
        ...
        print(" CSV export skipped (csv module not available)")
